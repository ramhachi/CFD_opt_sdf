"""PQ3.3b v9 entry preflight: one b=16 step at the v8 final checkpoint.

Verifies the v9 registration (budget extension only; policy and thresholds
unchanged) and runs exactly one cap-corrected inequality step at the v8 final
rho with fresh real primals. Append-only artifact; the campaign is refused
until it passes.
"""
...
OUTPUT = ROOT / "work/pq3_3b_v9_entry_preflight"
ARTIFACT = ROOT / "docs/evidence/pq3_3b_v9_entry_preflight_2026_09.json"
...
def verify() -> tuple[dict, np.ndarray]:
    ...
    if ca.sha256_file(MANIFEST) != sidecar:
        raise SystemExit("v9 manifest sidecar mismatch")
    manifest = ca.load_json(MANIFEST)
    if manifest["status"] != "registered_preflight_pending":
        raise SystemExit("v9 manifest is not awaiting its entry preflight")
    if ca.sha256_file(__file__) != manifest["entry_preflight"]["script_sha256"]:
        raise SystemExit("v9 entry preflight script hash mismatch")
    ...
    if ca.python_source_tree_sha256(ROOT / "src/cfd_sdf") != manifest["source_tree_python_sha256"]:
        raise SystemExit("source tree differs from the v9 registration")
    ...
    if OUTPUT.exists() or ARTIFACT.exists():
        raise SystemExit("v9 entry preflight artifacts already exist")
...
def run() -> dict:
    ...
    artifact = {
        "kind": "pq3_3b_v9_entry_preflight",
        "schema_version": 1,
        ...
        "claims_not_supported": ["the long v9 campaign is not started by this artifact"],
    }
    if ARTIFACT.exists():
        raise SystemExit("v9 preflight artifact already exists")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PQ3.3b v9 entry preflight")
