"""PQ3.3b v15 entry preflight: support gate + response ladder at the trimmed rho.

Verifies the v15 registration and qualifies one projected-direction step with
the new support-clearance gate and response-level ladder at the trimmed v14
checkpoint. Transform checks confirm the start state: projected volume within
the cap, mean_nd within the bound and zero support violations. Append-only
artifact; the campaign is refused until it passes.
"""
...
OUTPUT = ROOT / "work/pq3_3b_v15_entry_preflight"
ARTIFACT = ROOT / "docs/evidence/pq3_3b_v15_entry_preflight_2026_09.json"
...
def verify() -> tuple[dict, np.ndarray]:
    ...
    if ca.sha256_file(MANIFEST) != sidecar:
        raise SystemExit("v15 manifest sidecar mismatch")
    manifest = ca.load_json(MANIFEST)
    if manifest["status"] != "registered_preflight_pending":
        raise SystemExit("v15 manifest is not awaiting its entry preflight")
    if ca.sha256_file(__file__) != manifest["entry_preflight"]["script_sha256"]:
        raise SystemExit("v15 entry preflight script hash mismatch")
    ...
    if ca.python_source_tree_sha256(ROOT / "src/cfd_sdf") != manifest["source_tree_python_sha256"]:
        raise SystemExit("source tree differs from the v15 registration")
    ...
    if OUTPUT.exists() or ARTIFACT.exists():
        raise SystemExit("v15 entry preflight artifacts already exist")
...
def _finalize(manifest: dict, transform_checks: dict, stage: dict | None, stage_pass: bool) -> dict:
    ...
    artifact = {
        "kind": "pq3_3b_v15_entry_preflight",
        "schema_version": 1,
        ...
        "claims_supported": [
            "the trimmed start state satisfies the cap, the discreteness bound and zero support violations",
            "one projected-direction step was qualified with the support gate and the response-level ladder",
        ],
        "claims_not_supported": [
            "the long v15 campaign is not started by this artifact",
        ],
    }
    if ARTIFACT.exists():
        raise SystemExit("v15 preflight artifact already exists")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PQ3.3b v15 entry preflight")
