"""Register the Stage S baseline v2: repaired detector and surface-nets extraction.

The v1 baseline (``stage_s_baseline_v16_2026_09.json``) was bound to a PQ4.1
judgment made with the legacy marching-cubes extraction and the pre-repair
self-intersection detector. The topology-aware detector and the surface-nets
extraction repair changed both inputs, so the baseline is re-bound append-only
to the v2 PQ4.1 judgment; the v1 record stays unmodified. No solver run.
"""
...
PQ41_ARTIFACT = ROOT / "docs/evidence/pq4_1_v16_state_stage_s_entry_v2_2026_09.json"
PQ41_BUNDLE = ROOT / "work/pq4_1_v16_state_v2"
SELECTED_MANIFEST = PQ41_BUNDLE / "sweep/threshold_0.5/handoff_manifest.json"
GATE_SPEC = ROOT / "work/pq4_candidate/project.yaml"
SUPERSEDED = ROOT / "docs/evidence/stage_s_baseline_v16_2026_09.json"
ARTIFACT = ROOT / "docs/evidence/stage_s_baseline_v16_v2_2026_09.json"
...
def build() -> dict:
    if ARTIFACT.exists():
        raise SystemExit("Stage S baseline v2 artifact already exists; refuse overwrite")
    ...
    if verdict["ready_for_stage_s"] is not True:
        raise SystemExit("PQ4.1 v2 verdict is not ready_for_stage_s=true")
    if verdict["selected_threshold"] != 0.5:
        raise SystemExit("PQ4.1 v2 selected threshold is not the registered iso 0.5")
    if pq41["selection_rule"].get("require_ready_for_stage_s") is not True:
        raise SystemExit("PQ4.1 v2 selection rule does not require ready_for_stage_s")
    if pq41["input"]["campaign_manifest_sha256"] != _sha256(V16_MANIFEST):
        raise SystemExit("PQ4.1 v2 input does not match the v16 manifest")
    if pq41["input"]["campaign_outcome"]["sha256"] != _sha256(V16_OUTCOME):
        raise SystemExit("PQ4.1 v2 input does not match the v16 outcome")
    ...
    for key, record in handoff["artifacts"].items():
        ...
        handoff_artifacts[key] = {"path": str(path.relative_to(ROOT)), "sha256": actual}
    ...
    artifact = {
        "kind": "stage_s_baseline_v16_v2",
        "schema_version": 1,
        "baseline_id": "stage_s_baseline_v16_v2_2026_09",
        "supersedes": {
            "path": str(SUPERSEDED.relative_to(ROOT)),
            "sha256": _sha256(SUPERSEDED),
            "note": (
                "kept unmodified; its PQ4.1 judgment used the legacy marching-cubes "
                "extraction and the pre-repair self-intersection detector"
            ),
        },
        "change": {
            "id": "topology-aware-detector+surface-nets-extraction",
            "description": (
                "the direct self-intersection gate now handles coplanar area overlap and "
                "shared-vertex crossings and rejects degenerate triangles fail-closed; the "
                "handoff extracts the binary cell material with VTK surface nets "
                "(contour_labels, no smoothing), which emits no degenerate triangles. Both "
                "changes alter the selected surface, so the baseline is re-bound."
            ),
        },
        "candidate": {
        ...
        "claims_supported": [
            "the repaired composite Stage S entry gate returns ready_for_stage_s=true at iso 0.5 on the v16 checkpoint with the surface-nets extraction",
