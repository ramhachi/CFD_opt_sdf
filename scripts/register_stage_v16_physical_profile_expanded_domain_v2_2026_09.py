#!/usr/bin/env python3
"""Register one same-profile domain expansion after the V1 outer-field No-Go."""
...
BASE_SPEC = ASSET_ROOT / "project_matched_re_laminar_moving_ground_far_field_v2_expanded_domain_v1.yaml"
EXPANDED_SPEC = ASSET_ROOT / "project_matched_re_laminar_moving_ground_far_field_v2_expanded_domain_v2.yaml"
CANDIDATE = ASSET_ROOT / "v16_candidate_threshold_0p5.iso_surface.stl"
DESIGN_DOMAIN = ASSET_ROOT / "geometry/design_domain.stl"
PARENT_MANIFEST = ROOT / "docs/evidence/stage_v_v16_physical_profile_expanded_domain_manifest_v1_2026_09.json"
PARENT_RUN_MANIFEST = ROOT / "docs/evidence/stage_v_v16_physical_profile_expanded_domain_run_manifest_v1_2026_09.json"
PARENT_OUTCOME = ROOT / "docs/evidence/stage_v_v16_physical_profile_expanded_domain_v1_2026_09.json"
RUN_MANIFEST = ROOT / "docs/evidence/stage_v_v16_physical_profile_expanded_domain_v2_run_manifest_v1_2026_09.json"
MANIFEST = ROOT / "docs/evidence/stage_v_v16_physical_profile_expanded_domain_v2_manifest_v1_2026_09.json"
CASE_DIR = ROOT / "work/stage_v_v16_physical_profile_expanded_domain_v2_2026_09/case"
...
def _documents(
    ...
    question = "Does increasing only the outer-domain clearance remove the v2 physical-profile pressure disturbance?"
    execution = {
    ...
    run_document = {
        "kind": "stage_v_v16_physical_profile_expanded_domain_run_manifest",
        "schema_version": 1,
        "immutable": True,
    ...
    manifest = {
        "kind": "stage_v_v16_physical_profile_expanded_domain_manifest",
        "schema_version": 1,
        "immutable": True,
        "registered_before_computation": True,
        "status": "registered_not_run",
        ...
        "parent_v1": run_document["parent_v1"],
        "domain_factor": physics,
...
def register() -> dict[str, Any]:
    if any(path.exists() for path in (RUN_MANIFEST, MANIFEST)):
        raise SystemExit("expanded-domain manifest already exists; refuse overwrite")
    ...
    if parent_manifest.get("status") != "registered_not_run":
        raise SystemExit("parent v1 run manifest is not immutable registered_not_run")
    if parent.get("status") != "fail" or parent.get("qualified") is not False:
        raise SystemExit("expanded-domain correction requires the measured parent v1 No-Go")
    required_passes = (
        "identity",
        "candidate_clearance",
        "mesh_qualification",
    ...
    if parent["gates"]["outer_patch_backflow_and_pressure_disturbance"].get("status") != "fail":
        raise SystemExit("parent failure is not the registered outer-field failure")
...
def verify() -> dict[str, Any]:
    ...
    if run_document.get("status") != "registered_not_run" or manifest.get("status") != "registered_not_run":
        raise SystemExit("expanded-domain contract is not registered_not_run")
    for name, ref in run_document["source_artifacts"].items():
        if _sha256(ROOT / ref["path"]) != ref["sha256"]:
            raise SystemExit(f"registered expanded-domain source changed: {name}")
    for path in (RUN_MANIFEST, MANIFEST):
        sidecar = path.with_suffix(path.suffix + ".sha256")
        if _sha256(path) != sidecar.read_text(encoding="utf-8").strip():
            raise SystemExit(f"expanded-domain sidecar mismatch: {_rel(sidecar)}")
    materialization = _materialize_and_validate()
    if materialization["physical_profile_sha256"] != run_document["canonical_inputs"]["physical_profile_sha256"]:
        raise SystemExit("expanded-domain materialization changed the physical-profile hash")
    result = {
        "status": "pass",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="register one same-profile expanded-domain Stage V contract")
