#!/usr/bin/env python3
"""Run exactly one registered v2 Stage V physical-profile qualification case."""
...
import register_stage_v16_physical_profile_qualification_v1_2026_09 as registration  # noqa: E402


MANIFEST = registration.MANIFEST
RUN_MANIFEST = registration.RUN_MANIFEST
PROFILE_SPEC = registration.PROFILE_SPEC
CANDIDATE = registration.CANDIDATE
CASE_DIR = registration.CASE_DIR
OUTCOME = ROOT / "docs/evidence/stage_v_v16_physical_profile_qualification_v1_2026_09.json"
...
def _artifact_if_present(path: Path) -> dict[str, str] | None:
    return {"path": _rel(path), "sha256": _sha256(path)} if path.is_file() else None


def _write_outcome(document: dict[str, Any]) -> str:
    if OUTCOME.exists():
        raise SystemExit(f"qualification outcome already exists; refuse overwrite: {_rel(OUTCOME)}")
...
def _docker_image_id() -> str:
    ...
    if docker is None:
        raise SystemExit("docker is required for the registered controlled run")
    ...
    if completed.returncode != 0 or not completed.stdout.strip():
        raise SystemExit("registered OpenFOAM Docker image is unavailable")
...
def _verify_registration(run_manifest: dict[str, Any]) -> None:
    ...
    if run_manifest["execution"]["new_run_count"] != 0 or run_manifest["execution"]["solver_started"]:
        raise SystemExit("registered run manifest already records an execution")
    if _docker_image_id() != run_manifest["docker"]["image_id"]:
        raise SystemExit("Docker image identity differs from the registered image")
    if OUTCOME.exists():
        raise SystemExit("qualification outcome already exists; refusing a second run")
    if CASE_DIR.exists():
        raise SystemExit(f"case directory already exists; refusing to overwrite evidence: {_rel(CASE_DIR)}")
...
def _identity_gate(case_dir: Path, run_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    observed_candidate = _sha256(case_dir / "constant/triSurface/design_candidate.stl")
    checks = {
        "candidate_sha256": observed_candidate == expected["candidate"]["sha256"],
        ...
        "profile_spec_source": metadata.get("source_project") == str(PROFILE_SPEC.resolve()),
        "candidate_boundary_semantics": "design_candidate" in metadata.get("objective", {}).get("force_patches", []),
    }
    # The mesh boundary file is created by blockMesh inside the controlled
    # run.  Before that command, the registered boundary contract is present
    # in the generated blockMeshDict, which is the correct no-launch check.
...
def run(timeout_seconds: int) -> dict[str, Any]:
    ...
    if not preflight.qualified:
        outcome = {
            "kind": "stage_v_v16_physical_profile_qualification",
            "schema_version": 1,
    ...
    outcome = {
        ...
        "artifacts": {
            name: ref
            for name, path in {
                "case_metadata": CASE_DIR / "case_metadata.json",
                "mesh_log": mesh_log,
                "solver_log": solver_log,
                "run_summary": CASE_DIR / "openfoam_run_summary.json",
            }.items()
            if (ref := _artifact_if_present(path)) is not None
        },
        "claims_not_supported": run_manifest["claims_not_supported"],
        "next_gate": "same-profile domain convergence requires at least two domains and is not run by this slice" if passed else "remain blocked; fix the failed physical-profile gate before any domain comparison or Stage S work",
    }
    digest = _write_outcome(outcome)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="run the single registered v2 Stage V physical-profile qualification")
