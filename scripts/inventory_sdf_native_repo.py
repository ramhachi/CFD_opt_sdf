#!/usr/bin/env python3
"""Deterministic, solver-free inventory for the SDF-native architecture fork.

Records what existed at the fork so later design changes keep provenance of
what was replaced and what was preserved.  Reads files and hashes them only;
never starts a solver or mutates existing evidence.
"""
...
OUTPUT = ROOT / "docs/evidence/repo_inventory_sdf_native_v1.json"
...
SDF_SYMBOL_MODULES = ("src/cfd_sdf/sdf.py", "src/cfd_sdf/design/sdf_state.py")
...
def _documents() -> dict[str, Any]:
    ...
    return {
        "kind": "repo_inventory_sdf_native_v1",
        "schema_version": 1,
        "immutable": True,
...
def register() -> dict[str, Any]:
    if OUTPUT.exists():
        raise SystemExit("repo inventory already exists; refuse overwrite")
    digest = _write_immutable(OUTPUT, _documents())
    result = {"status": "pass", "inventory": OUTPUT.relative_to(ROOT).as_posix(), "sha256": digest, "solver_started": False}
...
def verify() -> dict[str, Any]:
    if not OUTPUT.is_file():
        raise SystemExit("repo inventory is missing")
    stored = json.loads(OUTPUT.read_text(encoding="utf-8"))
    if stored != _documents():
        raise SystemExit("repo inventory is not reproducible from the current tree")
    sidecar = OUTPUT.with_suffix(OUTPUT.suffix + ".sha256")
    if not sidecar.is_file() or sidecar.read_text(encoding="utf-8").strip() != _sha256(OUTPUT):
        raise SystemExit("repo inventory sidecar mismatch")
    result = {"status": "pass", "inventory": OUTPUT.relative_to(ROOT).as_posix(), "sha256": _sha256(OUTPUT), "solver_started": False}
    print(json.dumps(result, sort_keys=True))
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="register/verify the SDF-native repo inventory")
