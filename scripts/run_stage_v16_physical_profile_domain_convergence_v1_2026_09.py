#!/usr/bin/env python3
"""Run the single registered same-profile convergence-domain case."""
...
import register_stage_v16_physical_profile_domain_convergence_v1_2026_09 as registration  # noqa: E402
...
OUTCOME = ROOT / "docs/evidence/stage_v_v16_physical_profile_domain_convergence_v1_2026_09.json"
...
def _write_outcome(document: dict[str, Any]) -> str:
    if OUTCOME.exists():
        raise SystemExit(f"convergence-domain outcome already exists; refuse overwrite: {_rel(OUTCOME)}")
...
def _verify_registration(run_manifest: dict[str, Any]) -> None:
    ...
    if run_manifest["execution"]["new_run_count"] != 0 or run_manifest["execution"]["solver_started"]:
        raise SystemExit("convergence-domain run manifest already records an execution")
    if _docker_image_id() != run_manifest["docker"]["image_id"]:
        raise SystemExit("Docker image identity differs from the convergence-domain registration")
    if OUTCOME.exists():
        raise SystemExit("convergence-domain outcome already exists; refusing a second run")
    if CASE_DIR.exists():
        raise SystemExit(f"convergence-domain case already exists; refusing overwrite: {_rel(CASE_DIR)}")
...
def _identity_gate(case_dir: Path, run_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "expanded_problem_id": metadata.get("problem_id") == "stage_sv_v2_expanded_domain_v3",
...
def run(timeout_seconds: int) -> dict[str, Any]:
    ...
    if not preflight.qualified:
        outcome = {
            "kind": "stage_v_v16_physical_profile_domain_convergence",
            "schema_version": 1,
    ...
    outcome = {
        ...
        "run_manifest": {"path": _rel(RUN_MANIFEST), "sha256": _sha256(RUN_MANIFEST)},
        "manifest": {"path": _rel(MANIFEST), "sha256": _sha256(MANIFEST)},
        "parent_v2_outcome": {"path": run_manifest["parent_v2"]["outcome"]["path"], "sha256": run_manifest["parent_v2"]["outcome"]["sha256"]},
        ...
        "claims_not_supported": run_manifest["claims_not_supported"],
        "next_gate": "evaluate the registered v2-v3 same-profile convergence pair" if passed else "remain blocked; fix the failed physical-profile gate before convergence or Stage S",
    }
    digest = _write_outcome(outcome)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="run the single registered convergence-domain physical-profile case")
