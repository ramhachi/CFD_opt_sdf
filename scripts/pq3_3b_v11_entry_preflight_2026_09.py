"""PQ3.3b v11 entry preflight: one b=128 step at the v10 final checkpoint.

Verifies the v11 registration (budget extension only; policy and thresholds
unchanged) and runs exactly one cap-corrected b=128 inequality step at the v10
final rho with fresh real primals. Append-only artifact; the campaign is refused
until it passes.
"""
...
OUTPUT = ROOT / "work/pq3_3b_v11_entry_preflight"
ARTIFACT = ROOT / "docs/evidence/pq3_3b_v11_entry_preflight_2026_09.json"
...
def verify() -> tuple[dict, np.ndarray]:
    ...
    if ca.sha256_file(MANIFEST) != sidecar:
        raise SystemExit("v11 manifest sidecar mismatch")
    manifest = ca.load_json(MANIFEST)
    if manifest["status"] != "registered_preflight_pending":
        raise SystemExit("v11 manifest is not awaiting its entry preflight")
    if ca.sha256_file(__file__) != manifest["entry_preflight"]["script_sha256"]:
        raise SystemExit("v11 entry preflight script hash mismatch")
    ...
    if ca.python_source_tree_sha256(ROOT / "src/cfd_sdf") != manifest["source_tree_python_sha256"]:
        raise SystemExit("source tree differs from the v11 registration")
    ...
    if OUTPUT.exists() or ARTIFACT.exists():
        raise SystemExit("v11 entry preflight artifacts already exist")
...
def run() -> dict:
    ...
    passed = bool(payload["successful"])
    artifact = {
        "kind": "pq3_3b_v11_entry_preflight",
        "schema_version": 1,
        ...
        "claims_supported": ["one b=16 cap-corrected inequality step was qualified with fresh real primals"],
        "claims_not_supported": ["the long v11 campaign is not started by this artifact"],
    }
    if ARTIFACT.exists():
        raise SystemExit("v11 preflight artifact already exists")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PQ3.3b v11 entry preflight")
