...
5. **PQ4.1 — complete T-to-S handoff.** Extract `rho_projection`, retain
   ...
   **Reduced-basis architecture S0/S1 executed (2026-09-25).** Option 2 was
   ...
   next heavy step is the registered PQ2 Stage V domain/boundary factor before
   the S2 epsilon calibration.
...
7. **PQ2 — Stage V downforce reference (No-Go).** The registered V2 campaign
   is complete and both treatments move the responses beyond the registered
   bounds. Register a factor-resolved replacement contract, identify whether
   the large response is caused by domain placement, boundary formulation, or
   case-construction semantics, and rerun only the minimum result-driven
   family. Do not start S2 in parallel.
