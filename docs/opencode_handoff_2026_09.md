...
| P7 | Unresolved. Current injection overwrites `rho` while related arrays can retain an older generation; the current solver path reads `rho`, but future consumers could read stale state. |
