...
| P11 | Brinkman浸透層が格子で解像されていない | 最重大 | 測定中 |
| P12 | Stage V参照が未資格（mesh失敗・solver未収束） | 最重大 | 修正中 |
| P13 | 最適化ループの随伴が未収束のまま勾配に使われた | 最重大 | 修正中 |
| P14 | 射影がPythonとOpenFOAMで二重にかかる | 高 | 未修正 |
...
これは係数が誤りである証明ではないが、固体内の速度漏れとstiffnessが格子依存になり、
「Stage Tの固体」が実際には透過体である可能性を示す。`alphaMax`を上げれば漏れは減るが
収束性が悪化するため、**漏れ・収束・順位の基準を同時に満たす範囲が存在するか**が論点である。
存在しなければ、その格子でBrinkman surrogateはNo-Goとなる。

診断手順は`problem_resolution_plan_2026_09.md`§6.4。
